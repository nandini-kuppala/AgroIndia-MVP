...
const Analysis = () => {
  ...
  // Load field data when component mounts or fieldId changes
  useEffect(() => {
    if (fieldId) {
      fetchFieldData();
      fetchLatestAnalysis();
  ...
  const fetchFieldData = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        if (!data.coordinates) {
          toast.info("Please draw the field boundary on the map first");
          setShowMap(true);
        }
      }
    } catch (error) {
          ...
          <Card className="shadow-medium">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-muted-foreground mb-6">
                Select a field from Field Management to start analyzing
              </p>
            </CardContent>
          </Card>
        )}
